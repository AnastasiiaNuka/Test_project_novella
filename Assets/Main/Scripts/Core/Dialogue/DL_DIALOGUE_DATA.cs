...
// ласс разбивает текст диалога на сегменты, каждый из которых может содержать сигнал начала ({c}, {a}, {wc}, {wa}), задержку сигнала и соответствующий текст .
//ѕолученные сегменты хран€тс€ в списке дл€ последующей обработки.
{
    ...
    public List<DIALOGUE_SEGMENT> RipSegments(string rawDialogue)
    {
        ...
        for (int i = 0; i < matches.Count; i++)
        {
            ...
            segment.startSignal = (DIALOGUE_SEGMENT.StartSignal)Enum.Parse(typeof(DIALOGUE_SEGMENT.StartSignal), signalSplit[0].ToUpper()); // get the signal delay
